...
resource "oci_core_instance" "EnterpriseWebserver" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      echo "INFO: Testing SSH connectivity for WebServer${count.index + 1}..."
      for i in {1..10}; do
        if ssh -i ./id_rsa_enterprise -o ConnectTimeout=10 -o StrictHostKeyChecking=no ubuntu@${self.public_ip} "echo 'SSH OK'" 2>/dev/null; then
          echo "INFO: SSH connection established on attempt $i for WebServer${count.index + 1}"
          break
        else
          echo "INFO: SSH attempt $i failed for WebServer${count.index + 1}, waiting 10 seconds..."
          sleep 10
          if [ $i -eq 10 ]; then
            echo "ERROR: Could not establish SSH connection after 10 attempts for WebServer${count.index + 1}"
            exit 1
          fi
        fi
      done
      ...
      echo '${self.public_ip} ansible_user=ubuntu ansible_ssh_private_key_file=./id_rsa_enterprise' >> inventory_${count.index + 1}
