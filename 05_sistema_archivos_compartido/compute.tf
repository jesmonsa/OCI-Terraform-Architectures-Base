...
resource "oci_core_instance" "EnterpriseWebserver" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      if ansible-playbook -i inventory_webserver_${count.index + 1} -e "mount_target_ip=${oci_file_storage_mount_target.EnterpriseMountTarget.ip_address}" playbook.yml; then
