...
resource "null_resource" "webserver_provisioner" {
  ...
  depends_on = [
    ...
    oci_database_db_system.EnterpriseDBSystem  # Asegura que la base de datos esté lista
  ]
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      if ansible-playbook -i inventory_webserver_${count.index + 1} --extra-vars "MountTargetIPAddress=${var.MountTargetIPAddress}" --extra-vars "db_host=${data.oci_core_vnic.EnterpriseDBSystem_VNIC1.private_ip_address}" playbook.yml; then
