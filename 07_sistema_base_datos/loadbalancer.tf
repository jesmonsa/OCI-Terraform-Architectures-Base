...
resource "oci_load_balancer" "EnterpriseLoadBalancer" {
  ...
  network_security_group_ids = [oci_core_network_security_group.EnterpriseLBSecurityGroup.id]
...
# LoadBalancer Backendset con health-check corregido
resource "oci_load_balancer_backendset" "EnterpriseLoadBalancerBackendset" {
  ...
  health_checker {
    ...
    protocol            = "HTTP"
    response_body_regex = ".*"
    url_path            = "/healthz"  # Simple health check endpoint
    interval_ms         = 30000
...
# LoadBalancer Backend for WebServer Instances
