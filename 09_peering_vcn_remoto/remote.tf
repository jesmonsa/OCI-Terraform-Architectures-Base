...
resource "null_resource" "EnterpriseWebserverSharedFilesystem" {
  count = var.ComputeCount
  triggers = {
    instance_id = oci_core_instance.EnterpriseWebserver[count.index].id
  }  
  depends_on = [oci_core_instance.EnterpriseWebserver,oci_core_instance.EnterpriseBastionServer, oci_file_storage_export.EnterpriseExport]

  provisioner "remote-exec" {
    connection {
      type                = "ssh"
      user                = "opc"
      host                = data.oci_core_vnic.EnterpriseWebserver_VNIC1[count.index].private_ip_address
      private_key         = tls_private_key.public_private_key_pair.private_key_pem
      script_path         = "/home/opc/myssh.sh"
      agent               = false
      timeout             = "10m"
      bastion_host        = data.oci_core_vnic.EnterpriseBastionServer_VNIC1.public_ip_address
      bastion_port        = "22"
      bastion_user        = "opc"
      bastion_private_key = tls_private_key.public_private_key_pair.private_key_pem
    }
    inline = [
      "echo '== Start of null_resource.EnterpriseWebserverSharedFilesystem'",
      ...
      "echo '== End of null_resource.EnterpriseWebserverSharedFilesystem'"
    ]
  }
...
resource "null_resource" "EnterpriseWebserverHTTPD" {
  ...
  depends_on = [oci_core_instance.EnterpriseWebserver, oci_core_instance.EnterpriseBastionServer, null_resource.EnterpriseWebserverSharedFilesystem]
  ...
  provisioner "remote-exec" {
    ...
    inline = ["echo '== 1. Installing HTTPD package with dnf'",
      ...
      "sudo /bin/su -c \"echo 'Welcome to Enterprise.com! These are both WEBSERVERS under LB umbrella with shared index.html ...' > /sharedfs/index.html\"",
...
resource "null_resource" "EnterpriseWebserver_oci_iscsi_attach" {
  count = var.ComputeCount
  triggers = {
    instance_id = oci_core_instance.EnterpriseWebserver[count.index].id
  }  
  depends_on = [
    oci_core_instance.EnterpriseWebserver, 
    oci_core_instance.EnterpriseBastionServer, 
    null_resource.EnterpriseWebserverSharedFilesystem, 
    null_resource.EnterpriseWebserverHTTPD,
    oci_core_volume_attachment.EnterpriseWebserverBlockVolume_attach
  ]
...
resource "null_resource" "EnterpriseWebserver_oci_u01_fstab" {
  count = var.ComputeCount
  triggers = {
    instance_id = oci_core_instance.EnterpriseWebserver[count.index].id
  } 
  depends_on = [null_resource.EnterpriseWebserver_oci_iscsi_attach]
  ...
  provisioner "remote-exec" {
    ...
    inline = ["echo '== Start of null_resource.EnterpriseWebserver_oci_u01_fstab'",
      ...
      "echo '== End of null_resource.EnterpriseWebserver_oci_u01_fstab'",
    ]
  }
