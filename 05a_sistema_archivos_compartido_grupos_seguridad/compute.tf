...
resource "oci_core_instance" "EnterpriseWebserver" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      for i in {1..10}; do
        if ssh -i "$KEY_FILE" -o ConnectTimeout=10 -o StrictHostKeyChecking=no -o ProxyCommand="ssh -i $KEY_FILE -o StrictHostKeyChecking=no -W %h:%p ubuntu@${data.oci_core_vnic.EnterpriseBastionServer_VNIC1.public_ip_address}" ubuntu@${self.private_ip} "echo 'SSH OK'" 2>/dev/null; then
      ...
      echo '${self.private_ip} ansible_ssh_private_key_file='$KEY_FILE' ansible_ssh_common_args="-o ProxyCommand=\"ssh -i '$KEY_FILE' -o StrictHostKeyChecking=no -W %h:%p ubuntu@${data.oci_core_vnic.EnterpriseBastionServer_VNIC1.public_ip_address}\""' >> inventory_webserver_${count.index + 1}
...
resource "oci_core_instance" "EnterpriseBastionServer" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      echo '${self.public_ip} ansible_ssh_private_key_file='$KEY_FILE >> inventory_bastion
